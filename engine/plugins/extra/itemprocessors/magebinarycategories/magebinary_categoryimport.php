<?php
/**
 * This plugin will import and create the categories but skip the rows which already assigned with any category
 *
 * @todo The class should extend CategoryImporter to avoid code duplication but looks like we will have to change the Magmi core code to make it to be loaded correctly as a separate plugin
 */
class MageBinaryCategoryImporter extends Magmi_ItemProcessor
{
    // Store map code => id
    ...
    {
        return array(
            "name"      => "MageBinary - On the fly category creator/importer",
            "author"    => "David Qian",
            "version"   => "0.0.1",
    ...
    {
        $categoryTable = $this->tablename('catalog_category_product');
        $sql = "SELECT cat.category_id FROM $categoryTable as cat WHERE cat.product_id=?";
        $result = $this->selectone($sql, array($productId), "category_id");
        return $result;
    }
    ...
    public function processItemAfterId(&$item, $params = null)
    {
        if (isset($item["categories"])) {
            if ($this->getParam("CAT:nooverwrite", 1) == 1) {
                $itemData = $this->getItemIds($item);
                // if the item has an existing category
                if (isset($itemData['pid']) && !empty($this->getCategoryByProductId($itemData['pid']))) {
                    return true;
                }
            }
            // handle escaping
